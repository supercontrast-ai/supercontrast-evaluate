...
- **Metric**: A metric is used to evaluate a model's performance and usually involves the model's predictions as well as some ground truth labels. You can find all integrated metrics at [supercontrast_evaluate.Metric](https://huggingface.co/supercontrast_evaluate.Metric).
...
Each of these evaluation modules live on Hugging Face Hub as a Space. They come with an interactive widget and a documentation card documenting its use and limitations. For example [accuracy](https://huggingface.co/spaces/supercontrast_evaluate.Metric/accuracy):
