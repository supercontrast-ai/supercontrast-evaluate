...
Examples:
    ...
    >>> xtreme_s_metric = supercontrast_evaluate.load('xtreme_s', 'mls')  # 'mls', 'voxpopuli', 'fleurs-asr' or 'babel'
    ...
    >>> xtreme_s_metric = supercontrast_evaluate.load('xtreme_s', 'covost2')
    ...
    >>> xtreme_s_metric = supercontrast_evaluate.load('xtreme_s', 'fleurs-lang_id')
    ...
    >>> xtreme_s_metric = supercontrast_evaluate.load('xtreme_s', 'minds14')
    >>> references = [0, 1, 0, 0, 1]
    >>> predictions = [0, 1, 1, 0, 0]
    >>> results = xtreme_s_metric.compute(predictions=predictions, references=references)
    >>> print({k: round(v, 2) for k, v in results.items()})
